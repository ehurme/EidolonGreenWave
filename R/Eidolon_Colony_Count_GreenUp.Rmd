...
for(i in 1:length(IDs)){
  ...
  with(c, plot(month, count, type = "o", 
...
batdf_max$month <- month(batdf_max$date)
...
ggplot(data = Africa)+ geom_sf() +
  ...
  theme_classic() +
  viridis::scale_color_viridis(name = "Month")+
...
for(i in 1:length(IDs)){
  ...
  legend("bottomleft", legend = c("EVI", "IRG", "Precip", "EVAP", "Colony"), pch = 16, col = c(3,2,4,5,1))
...
p3 <- ggplot(Peaks, aes(y = yday(month), x = yday(aet_spline), col = abs(Lat)))+
  geom_point()+xlim(c(0,365))+ylim(c(0,365))+
  geom_smooth(method = "lm")+
