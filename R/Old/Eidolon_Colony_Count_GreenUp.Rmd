...
p_load(ggplot2, dplyr, tidyverse,
       scales, viridisLite, ggpubr, readxl, janitor, 
       zoo, circular, CircMLE, lubridate, # toolkit
       raster, rnaturalearth, rnaturalearthdata, # maps
       rgbif, rinat, # species occurrence
       mgcv, lme4, MuMIn, tidymodels, vip) # modeling
       # update = TRUE) 
...
# aet <- read.csv("../../../../Dropbox/GreenWave/EidolonColonies/rawdata/EvapoTranspiration/tcy_aet_2000_2020_ptsB.csv")
# def <- read.csv("../../../../Dropbox/GreenWave/EidolonColonies/rawdata/EvapoTranspiration/tcy_def_2000_2020_ptsB.csv")
# psdi <- read.csv("../../../../Dropbox/GreenWave/EidolonColonies/rawdata/EvapoTranspiration/tcy_pdsi_2000_2020_ptsB.csv")
# pet <- read.csv("../../../../Dropbox/GreenWave/EidolonColonies/rawdata/EvapoTranspiration/tcy_pet_2000_2020_ptsB.csv")
# soil <- read.csv("../../../../Dropbox/GreenWave/EidolonColonies/rawdata/EvapoTranspiration/tcy_soil_2000_2020_ptsB.csv")
# 
# evap <- data.frame(time = ymd_hms(aet$startDate), geeID = aet$geeID, aet = aet$mean, 
#                    def = def$mean, psdi = psdi$mean, pet = pet$mean, soil = soil$mean)
```
...
#                ylim = c(0,1), lwd = 2,
#                xlim = c(min(month)-365, max(month)+365),
...
#   #              pch = 16, col = as.numeric(is.na(Observer))+1))
#   #with(t, points(as.Date(time), aet/max(aet), type = "o", col = 2, pch = 16))
#   # with(t, points(as.Date(time), def/max(def), type = "o", col = 3, pch = 16))
#   # with(t, points(as.Date(time), psdi/max(psdi), type = "o", col = 4, pch = 16))
#   # with(t, points(as.Date(time), pet/max(pet), type = "o", col = 5, pch = 16))
#   #with(t, points(as.Date(time), soil/max(soil), type = "o", col = 6, pch = 16))
#   legend("bottomleft", legend = c("colony", "aet", "def", "psdi", "pet", "soil"),
#                                   # "EVI", "Precipitation", 
#                                   # "EvapoTrans"),
#          col = c(1:6), pch = 16)
# }
...
ggplot(data = Africa)+ geom_sf() +
  geom_point(data = batdf_max_mean, aes(x = Long, y = Lat, 
                               size = log10(peak_size), color = month)) +
  xlim(c(-20, 45))+ ylim(c(-20, 20)) + 
  theme_classic() +
  scale_color_gradient(low = "blue", high = "red", name = "Month")+
...
fig1
# ggsave(fig1, filename = "../../../../Dropbox/MPI/Eidolon/Greenwave/plots/fig1_avg_colony_map.png")
save(batdf_max_mean, file = "../../../../Dropbox/MPI/Eidolon/Greenwave/data/avg_colony_size.RData")
```
...
for(i in 1:length(IDs)){
  ...
  lines(pred, col=2)
  # lines(predt, col = 4)
  lines(as.Date(p$time), p$pred/max(p$pred), col = "darkblue")
  
  ## get derivative of EVI -> IRG
  pred.prime <- predict(spl, deriv=1)
  pred.prime$y_scale <- rescale(pred.prime$y, new.min = -1, new.max = 1)
...
summary(rs)
save(colonies, rs, file = "../../../../Dropbox/MPI/Eidolon/Greenwave/rdata/regular_colonies.Rdata")
load("../../../../Dropbox/MPI/Eidolon/Greenwave/rdata/regular_colonies.Rdata")
# (rs$daylength) %>% plot
```  
...
for(i in 1:length(IDs)){
  ...
  points(r$year, month(r$precip_pspline), type = "o", pch = 16, col = "darkblue")
  ...
  legend("bottomleft", legend = c("EVI", "IRG", "Precip", "PrpStart", 
                                  "Colony"), pch = 16, col = c(3,2,4,"darkblue",
                                                               1))
...
save(rs, rs_max, rs_max_sum, file = "../../../../Dropbox/MPI/Eidolon/Greenwave/rdata/rs_max_sum.Rdata")
load("../../../../Dropbox/MPI/Eidolon/Greenwave/rdata/rs_max_sum.Rdata")
```

# Table S3
```{r}
EVI_table <- rs %>% group_by(geeID) %>% 
  summarise(EVI = paste0(round(min_EVI[which.min(min_EVI)],2), " - ",
    round(max_EVI[which.max(max_EVI)],2)),
    IRG = paste0(round(min_EVI[which.min(min_EVI)],2), " - ",
    round(max_EVI[which.max(max_EVI)],2)))
# View(EVI_table)
```
...
points(rs_max_sum$geeID, rs_max_sum$precipp_intensity, type = "o", col = "darkblue")
```
```{r}
...
points(rs_max_sum$entropy, rs_max_sum$precipp_intensity, type = "o", col = "darkblue")
```

# Add start and end of season estimates to each peak
## do we want to reduce resolution of all points?
```{r}
# Peaks$sos <- NA
# Peaks$eos <- NA
...
#<<<<<<< HEAD
#   
#   while(any(is.na(Peaks$sos[idx]))){
#     Peaks$sos[idx] <- raster::extract(x = sos, y = xy)
#     Peaks$eos[idx] <- raster::extract(eos, xy)
#     
#=======
# 
#   while(any(is.na(Peaks$sos[idx]))){
...
#     Peaks$eos[idx] <- raster::extract(eos, xy)
# 
#>>>>>>> 8b456203e316170af112c071b4ff49be51c2f9c6
#     sos <- aggregate(sos, fact = 10)
#     eos <- aggregate(eos, fact = 10)
#   }
#   Peaks$sos_res[idx] <- res(sos)[1]
#   print(res(sos))
#   print(res(eos))
#   # plot(sos2)
#   # plot(eos2)
...
# 
# save(Peaks, rs, file = "../../../../Dropbox/MPI/Eidolon/Greenwave/rdata/peaks_sos.RData")
# load("../../../../Dropbox/MPI/Eidolon/Greenwave/rdata/peaks_sos.RData")
...
## one map
```{r}
circ.intensity.month(month(round_date(Peaks$month[Peaks$geeID == 2], unit = "months")))

avg_peaks <- 
  Peaks %>% group_by(geeID, Location) %>% 
  summarise(count = mean(Count), Lat = Lat[1], Long = Long[1], 
            month = round(circ.mean.month(month(round_date(date, unit = "months"))),0), 
            intensity = round(circ.intensity.month(month(round_date(date, unit = "months"))),2), 
            n = n())
avg_peaks
plot(rs_max_sum$EVI_intensity, avg_peaks$intensity, cex = avg_peaks$n)
plot(rs_max_sum$IRG_intensity, avg_peaks$intensity, cex = avg_peaks$n)
```
...
# Peaks$aet_spline <- ymd("1900-01-01")
...
for(i in 1:length(IDs)){
  ...
  for(j in 1:nrow(p)){
    ...
    Peaks$precip_pspline[pidx[j]] <- round_date(
      ...
      unit = "month")
    # # evapotranspiration
    #     Peaks$aet_spline[pidx[j]] <- round_date(
    #   r$aet_spline[which.min(abs(p$month[j] - as.Date(r$aet_spline)))], 
    #   unit = "month")
...
Peaks$months_to_precip_pspline <- round(difftime(Peaks$month, 
                                                ...
                                                unit = "weeks")/4,0) %>% as.numeric
# Peaks$months_to_aet_spline <- round(difftime(Peaks$month, 
#                                                 Peaks$aet_spline, 
#                                                 unit = "weeks")/4,0) %>% as.numeric
...
save(avg_peaks, Peaks, Strict_Peaks, rs, file = "../../../../Dropbox/MPI/Eidolon/Greenwave/rdata/avg_peaks.RData")
...
for(i in 1:nrow(Peaks)){
  ...
  lines(predp.prime$x, predp.prime$y01, col = "darkblue")
  ...
  abline(v = p$precip_pspline, col = "darkblue")
...
for(i in idx){
  ...
  pred.prime <- predict(spl, deriv = 1)
  pred.prime$y01 <- rescale(pred.prime$y, new.min = -0.5, new.max = 0.5)
  plot(p$month, p$ratio, col = 0, frame.plot = FALSE,
       xlim = c(ymd("2014-01-01"), ymd("2016-01-01")),
       # xlim = c(min(p$month) - years(1)-months(1), 
       #          max(p$month) + years(1)+months(1)), 
       ylim = c(-0.5, 1), 
       xlab = "time",
       ylab = "EVI/ratio", 
       main = paste0(p$Location[1], ", Entropy: ", round(p$entropy, 2)))
  abline(v = seq.Date(as.Date("2000-01-01"), 
                      as.Date("2021-01-01"), by = "year"), 
         col = "gray", lty = 2)
  abline(h = 0)
...
plot(month(Peaks$precip_pspline), month(Peaks$IRG_spline))
cor(sin(pi*month(Peaks$precip_pspline)/12), sin(pi*month(Peaks$IRG_spline)/12))
```
...
p4 <- ggplot(Peaks, aes(y = yday(month), x = yday(precip_pspline), col = abs(Lat)))+
  geom_point()+xlim(c(0,365))+ylim(c(0,365))+
  geom_smooth(method = "lm")+
  geom_abline(slope = 1, intercept = 0)+ylab("Peak Colony Day")+xlab("Peak Precipitation Day")+
  scale_color_viridis_c(name='abs(Latitude)', option='E', alpha=0.7)+theme_classic()+
...
round(cor.circular(y = pi*month(Peaks$month)/12, 
                   x = pi*month(Peaks$EVI_model)/12), 2)
round(cor.circular(y = pi*month(Peaks$month)/12, 
                   x = pi*month(Peaks$EVI_spline)/12), 2)
round(cor.circular(y = pi*month(Peaks$month)/12, 
                   x = pi*month(Peaks$IRG_model)/12), 2)
round(cor.circular(y = pi*month(Peaks$month)/12, 
...
p4 <- 
  ggplot(Peaks, aes(y = month(month), 
                             ...
                             # col = abs(Lat)))+
  geom_point(col = "darkblue")+xlim(c(0,12))+ylim(c(0,12))+
  # geom_smooth(method = "lm", formula = my.formula)+
  geom_abline(slope = 1, intercept = 0)+
    ylab("Peak Colony Month")+xlab("Peak Start Precip Month")+
  # scale_color_viridis_c(name='abs(Latitude)', option='C', alpha=0.7)+
    theme_classic()+
    scale_y_continuous(breaks=c(3,6,9,12), limits = c(0,12))+
    scale_x_continuous(breaks=c(3,6,9,12), limits = c(0,12))+
...
my_comparisons <- list( c("Peak EVI", "Peak IRG"), c("Peak EVI", "Peak Precip"), c("Peak IRG", "Peak Precip") )
p4a <- ggviolin(CC_mlt, x = "variable", y = "value", fill = "variable",
         palette = cbPalette, 
         ylim = c(-10,20),
...
p <- ggarrange(p1,p2,p3,p4a, ncol = 2, nrow = 2, common.legend = FALSE,
          labels = c("A", "B", "C", "D"), label.y = 1)  
p
ggsave(p, filename = "../../../../Dropbox/MPI/Eidolon/GreenWave/plots/fig3.png")
```
...
>>>>>>> f4273b6e6c1f7ce10ea8e48c24d9b72648569769
...
38/sum(table(Peaks$abs_months_to_IRG_spline))
```
## add lat long to rs
```{r}
rs$seasonality <- NA
...
for(i in 1:nrow(Peaks)){
  ...
  #Peaks$seasonality[i] <- max(rs$seasonality[idx]
...
```
```{r}
save(Peaks, rs, file = "../../../../Dropbox/MPI/Eidolon/Greenwave/rdata/model_peaks.RData")
...
psych::pairs.panels(Peaks[,c("abs_months_to_IRG_spline", "log_colony_size",
                             ...
                             "IRG_amp",
                             "amp_EVI", "amp", "LOS", "avg_LOS")])

summary(Peaks)
### add to supplement - corrplot
```
...
Peaks[which.max(Peaks$abs_months_to_IRG_spline),]
...
table(Peaks$abs_months_to_IRG_spline)
m0 <- glmer(abs(months_to_IRG_spline)~
             1+
             (1|geeID),
...
for(i in 1:length(IDs)){
  ...
  fits[[i]] <- lmer((abs_months_to_IRG_spline) ~ 
                     #scale(avg_LOS)+
                     scale(max_EVI)+
                     scale(entropy)+
                     scale(log_colony_size)+
...
m1 = glmer(abs(months_to_EVI_spline)~
             scale(log(count))+
             scale(max_EVI)+
             scale(amp)+
             scale(seasonality)+
