...
unique(paste0(Peaks$Country, ", ", Peaks$Location))
...
save(avg_peaks, Peaks, Strict_Peaks, reg_colonies, Africa, EVI, rs, file = "../../../../Dropbox/MPI/Eidolon/Greenwave/rdata/avg_peaks_TRMM.RData")
...
# threshold over 25mm/month
pm <- precip %>% group_by(geeID, year = year(time)) %>% 
  summarise(onset = time[which((mean*30*24) > 45)],
            precip = mean[which((mean*30*24) > 45)]*30*24)

pm$diff <- c(diff(pm$onset), days(0))
pm$seq <- 1
pm$seq[which(pm$diff > 31 | pm$diff < 28)] <- 0

k <- rle(pm$seq)

pm$start <- 0
pm$start[cumsum(k$lengths)[which(k$lengths >= 2)-1]+1] <- 1

pm$start[1] <- 1

onset <- pm[pm$start == 1,]

table(onset$geeID, onset$year)
table(precip$geeID, year(precip$time))
i = 15
j = 2
for(i in 15:17){
  o <- onset[onset$geeID == i,]
  years <- 2000:2020
  # for(j in 1:length(years)){
    # oy <- o[o$year == years[j],]
    py <- precip[which(precip$geeID == i),]
    py <- py[!is.na(py$mean),]
    plot(py$time, py$mean*30*24, type = "o", main = i)
    abline(v = o$onset)
  # }
}
...
## alternatively 45mm in one month would do it...
...
## distribution of peak EVI, IRG, and Precip months
```{r}
library(data.table)
...
p1 <- ggplot(Peaks, aes(y = month(month), x = month(EVI_spline), col = abs(Lat)))+
  geom_point()+xlim(c(1,12))+ylim(c(1,12))+
...
p2 <- ggplot(Peaks, aes(y = month(month), x = month(IRG_spline), col = abs(Lat)))+
  geom_point()+xlim(c(1,12))+ylim(c(1,12))+
  geom_smooth(method = "lm")+
...
p3 <- ggplot(Peaks, aes(y = month(month), x = month(precip_spline), col = abs(Lat)))+
  geom_point()+xlim(c(1,12))+ylim(c(1,12))+
  geom_smooth(method = "lm")+
...
p4 <- ggplot(Peaks, aes(y = month(month), x = month(precip_pspline), col = abs(Lat)))+
  geom_point()+xlim(c(1,12))+ylim(c(1,12))+
  geom_smooth(method = "lm")+
  geom_abline(slope = 1, intercept = 0)+ylab("Peak Colony Day")+xlab("Peak Precipitation Day")+
  scale_color_viridis_c(name='abs(Latitude)', option='E', alpha=0.7)+theme_classic()+
...
```
#### get mean and sd for each month diff
```{r}
Peaks$months_to_IRG_spline %>% summary
Peaks$months_to_precip_pspline %>% summary
...
p5 <- ggplot(CC_mlt, aes(x = variable, y = value, fill = variable)) + 
  geom_hline(yintercept = 0) + 
  geom_violin(draw_quantiles = c(0.5)) + theme_classic() + 
  geom_jitter(width = 0.2, alpha = 0.2)+
  theme(legend.position = "none") + 
  scale_fill_manual(values=c(cbPalette, "turquoise"))+
  # scale_fill_viridis_d() +
  ylab("Months to Peak Colony Size") + xlab("")+ylim(c(-10,7))


ggarrange(p1,p2,p3,p4,p5, ncol = 2, nrow = 3, common.legend = FALSE, widths = c(1,1,1,1,2))  
...
my_comparisons <- list( c("Peak EVI", "Peak IRG"), 
                        ...
                        c("Peak IRG", "Peak PRP"),
                        c("Peak IRG", "Peak IRP"), 
                        c("Peak Precip", "Peak IRP"), 
                        c("Peak EVI", "Peak IRP"))
p5a <- ggviolin(CC_mlt, x = "variable", y = "value", fill = "variable",
...
p5a+theme_get()+theme(legend.position = "none")
...
```{r}
table(Peaks$geeID)
idx <- Peaks$geeID %in% which(table(Peaks$geeID)>=4) 
...
m1 = lmer(abs(months_to_IRG_spline)~
             #scale(abs_Lat)+
             scale(entropy)+
             #scale(LOS)+
             #scale(avg_LOS)+
...
# overdispersion
check_overdispersion(m1_best)

# zero inflation
check_zeroinflation(m1_best)

# check singularity
check_singularity(m1_best)
isSingular(m1_best)

# check collinearity
check_collinearity(m1_best)

model_performance(m1_best)
?model_performance
compare_performance(m1_best, m0, m1, rank = TRUE)
```
