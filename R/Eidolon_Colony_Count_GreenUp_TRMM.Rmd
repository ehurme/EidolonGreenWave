...
table(Peaks$abs_months_to_IRG_spline)
m0 <- lm(abs(months_to_IRG_spline)~
             1,
             #(1|geeID),
...
m1 = lm(abs(months_to_IRG_spline)~
             #scale(abs_Lat)+
             scale(entropy)+
             #scale(LOS)+
             #scale(avg_LOS)+
...
### best
```{r}
m1_best = lm(abs(months_to_IRG_spline)~
             #scale(avg_LOS)+
             scale(max_EVI)+
               # scale(IRG_amp)+
             scale(entropy)+
...
compare_performance(m1_best, rank = TRUE)
```

## lmer
```{r}
l1_best = lm(abs(months_to_IRG_spline)~
             #scale(avg_LOS)+
             scale(max_EVI)+
               # scale(IRG_amp)+
             scale(entropy)+
...
for(i in 1:length(IDs)){
  ...
  fits[[i]] <- lm((abs_months_to_IRG_spline) ~ 
                     #scale(avg_LOS)+
                     scale(max_EVI)+
                     scale(entropy)+
                     scale(log_colony_size),
